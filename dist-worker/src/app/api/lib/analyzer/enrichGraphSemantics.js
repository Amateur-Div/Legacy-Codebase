...
exports.enrichGraphSemantics = enrichGraphSemantics;
...
function enrichGraphSemantics(graph) {
    const mergedNodes = graph.nodes;
    const mergedEdges = graph.edges;
    const nodes = mergedNodes.map((n) => ({ ...n, semantic: { ...(n.semantic || {}) } }));
    const edges = mergedEdges.map((e) => ({ ...e }));
    ...
    for (const n of nodes) {
        const deg = (inAdj.get(n.id) || []).length + (outAdj.get(n.id) || []).length;
        degrees.set(n.id, deg);
        if (deg > maxDegree)
            maxDegree = deg;
    }
    function estimateComplexity(n) {
        if (!n.code)
            return 1;
        const code = n.code;
        let c = 1;
        ...
        const nest = Math.max(0, (code.match(/{/g) || []).length - (code.match(/}/g) || []).length);
