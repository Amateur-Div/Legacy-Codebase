  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory) {
        allPaths.push(normalized.endsWith("/") ? normalized : normalized + "/");
        continue;
      }
    ...
    const fileTree = await (async function buildTree() {
      const tree: any[] = [];
        ...
        for (let i = 0; i < parts.length; i++) {
          ...
          if (!node) {
            node = {
              name: part,
              type: isFile ? "file" : "folder",
              fullPath: isFile ? file.path : undefined,
              children: isFile ? undefined : [],
            };
            current.push(node);
          }
          ...
          if (isFile) {
            ...
            if (["js", "jsx", "ts", "tsx"].includes(ext || "")) {
              try {
                const trackExecution = instrumentExecutionBabel(file.content);
                perFileGraphs.push({
                  file: file.path,
                  graph: trackExecution,
                });

                const symbols = extractStructureBabel(file.path, file.content);

                Object.assign(node, {
                  language: detectLanguage(file.path),
                  languageColor: getLanguageColor(detectLanguage(file.path)),
                  highlights: extractHighlights(file.content),
                  entry: isEntryFile(part, file.content),
                  ...symbols,
                });
              } catch (err) {
                console.warn("AST error:", err);
              }
            }
          } else {
    ...
    return NextResponse.json({
      message: "Project uploaded (in-memory)",
      projectId,
      jobId: job.id,
